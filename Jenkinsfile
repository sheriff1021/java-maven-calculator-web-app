	...
	stages{
		...
		stage("nexus"){
				steps{
					...
					sh "tar -cf calculator.tar.gz calculator.war"
					sh "curl -v --user 'admin:admin' --upload-file ./calculator.tar.gz http://localhost:8081/repository/maven-releases/com/company/sample-app/${BUILD_NUMBER}/calculator.tar"					
				}
		}
